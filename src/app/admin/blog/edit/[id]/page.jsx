...
export default function EditBlogPost() {
    ...
    const fetchBlogData = async () => {
        try {
            ...
            const formattedDate = format(new Date(blogData.date), 'yyyy-MM-dd');
            const normalizedContent = blogData.content.map((section, index) => ({
                ...section,
            ...
            setFormData({
                ...
                author: {
                    name: blogData.author?.name || session?.user?.name || '',
                    image: blogData.author?.image || session?.user?.image || ''
                }
            });

            setIsFetching(false);
        } catch (err) {
            setError('Error loading blog: ' + err.message);
            setIsFetching(false);
        }
    };
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <input
                                            ...
                                            className="hidden"
                                            required={!formData.author.image}
                                        />
                                        <label
                                            htmlFor="authorImage"
