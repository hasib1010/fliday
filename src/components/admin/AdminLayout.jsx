...
export default function AdminLayout({ children }) {
  ...
  useEffect(() => {
    if (pathname.startsWith('/admin/orders') || pathname.startsWith('/admin/topups')) {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-100 lg:flex">
        {/* Sidebar */}
        <aside
          className={`fixed lg:sticky top-0 left-0 z-50 w-64 bg-white shadow-lg transition-transform duration-300 lg:transform-none h-screen ${
                  ...
                  {mainNavigation.map((item) => {
                    ...
                    return (
                      <div key={item.name}>
                        {item.hasSubmenu ? (
                          ...
                          >
                            <div className="flex items-center">
                              <Icon
                                size={20}
                                className={`mr-3 ${
                                  isItemActive || isSubmenuOpen ? 'text-[#F15A25]' : 'text-gray-500'
                                }`}
                              />
                              {item.name}
                            </div>
                          ...
                          >
                            <Icon
                              size={20}
                              className={`mr-3 ${
                                isItemActive ? 'text-[#F15A25]' : 'text-gray-500'
                              }`}
                            />
                            {item.name}
                          </Link>
                        ...
                        {item.hasSubmenu && (
                          <div
                            className={`pl-10 mt-1 space-y-1 overflow-hidden transition-all duration-200 ${
                    ...
                    return (
                      ...
                      >
                        <Icon
                          size={18}
                          className={`mr-3 ${
                            isActive(item.href) ? 'text-[#F15A25]' : 'text-gray-500'
                          }`}
                        />
                        {item.name}
                      </Link>
            ...
            <div className="p-4 border-t border-gray-200 flex-shrink-0 bg-white">
              <div className="flex items-center">
                ...
                <button
                  ...
                  className="p-1 rounded-full text-gray-500 hover:text-red-600 hover:bg-gray-100 flex-shrink-0"
                  title="Sign out"
                >
                  <LogOut size={18} />
                </button>
